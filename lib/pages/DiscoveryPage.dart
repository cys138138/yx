...
import 'package:yx/domain/DiscoveryBean.dart';
...
//  短的分割线应该是左右margin+图标宽高+文字margin
  static const shortDivisionPaddingLeft =
      itemMarginLeftRight + itemResWidthHeight + textMarginLeftRight;

  DiscoveryPage() {
    var osSoft = new DiscoveryBean(
        'images/ic_discover_softwares.png', '开源软件', true, false,
        isLinkWebPage: true, linkUrl: 'https://zb.oschina.net/?from=osc');
    var recommend = new DiscoveryBean(
        'images/ic_discover_git.png', '码云推荐', false, false,
        isLinkWebPage: true, linkUrl: 'https://www.oschina.net/blog');
    ...
    var wave =
        new DiscoveryBean('images/ic_discover_shake.png', '摇一摇', false, true);
    var nearby = new DiscoveryBean(
        'images/ic_discover_nearby.png', '码云封面人物', true, false,
        isLinkWebPage: true,
        linkUrl:
            'https://www.oschina.net/question/tag/%E9%AB%98%E6%89%8B%E9%97%AE%E7%AD%94');
    var offline = new DiscoveryBean(
        'images/ic_discover_pos.png', '线下活动', false, true,
        isLinkWebPage: true, linkUrl: 'https://www.oschina.net/event');
...
// 定义发现页面列表
  ...
  Widget renderRow(BuildContext con, int index) {
    ...
    return new InkWell(
        ...
        children: <Widget>[
          new Container(
            ...
            margin: new EdgeInsets.fromLTRB(
                itemMarginLeftRight, marginTop, itemMarginLeftRight, 0.0),
            child: new Row(
              children: <Widget>[
                new Image.asset(
                  bean.menuRes,
                  width: itemResWidthHeight,
                  height: itemResWidthHeight,
                ),
                new Expanded(
                    child: new Padding(
                  padding: new EdgeInsets.fromLTRB(
                      textMarginLeftRight, 0.0, textMarginLeftRight, 0.0),
                  child: new Text(
                    bean.menuMsg,
                    style: menuStyle,
                  ),
      ...
      onTap: () {
        if(bean.isLinkWebPage){
          Navigator.of(con).push(new MaterialPageRoute(builder: (context){
            return new NewsDetailPage(bean.linkUrl,bean.menuMsg);
          }));
        }else{
